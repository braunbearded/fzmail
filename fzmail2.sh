...
if [ ! -f "$cache_path/current" ]; then
    ./fzcache.sh "clear" "$cache_path"
    ./fzcache.sh "init" "$cache_path" "$profiles"
    ./fzrender.sh "init" "$cache_path" > "$cache_path/current"
    ./fzrender.sh "fzf_header" "$cache_path" > "$cache_path/fzf_header"
fi
    # ./fzrender.sh "init" "$cache_path" > "$cache_path/current"

fzf_header="$(cat "$cache_path/fzf_header")"

while true; do
    entry="$(fzf --tac --header "$fzf_header" \
        --preview "./fzrender.sh preview $cache_path {}" \
        --preview-window "right:33%" -d "\|" --with-nth=2.. < "$cache_path/current")"
    entry="$(echo "$entry" | cut -d "|" -f 2-)"

    entry_type="$(echo "$entry" | cut -d "|" -f 4)"

    [ "$entry" = "" -o "$entry_type" = "Exit" ] && exit

    if [ -d "$entry_type" ]; then
    ...
    if [ -f "$entry_type" ]; then
        echo "mail"
    fi

    if [ "$entry_type" = "Expand all            " ]; then
